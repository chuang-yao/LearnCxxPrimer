...
template <typename T1, typename T2> double l2Norm(vector<T1> x, vector<T2> y) {
  auto itr_x = x.cbegin();
  auto itr_y = y.cbegin();
  double s = 0.0;
...
int main() {
...

  {
    ...
    // d = i = 3.5;
    cout << d << " " << i
         << endl; // WARNING: implicit conversion from 'double' to 'int'
  }

  {
    double dval;
    int ival;
    int *pi = &ival; // pi must be initialized before modifying the object it
                     // points to
    dval = ival = *pi = 0;
  }
...

  {
    const char *pc;
    char *p =
        const_cast<char *>(pc); // okay, but writing through p is undefined
  }

  {
    const char *cp;
    // char *q = static_cast<char*>(cp);  // error: static_cast cannot cast
    // aways const
    static_cast<string>(cp);
    // const_cast<string>(cp);  // the type in a const_cast must be a pointer,
